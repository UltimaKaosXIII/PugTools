...
namespace PugTools {
  internal sealed class WorldEffect : SlimDXNet.FX.Effect {
    public readonly EffectTechnique Lit, LocalLightAdd, AlphaTestLit, AlphaLit, AddLit, MultiplyLit, HookOverlay, HookAdditive, Unlit, Sky, TerrainCoverage, TerrainLit, TerrainAddLit, TerrainLocalLightAdd, TerrainUnlit, TerrainAddUnlit, Height, Wire, Water, Overlay, MapMarker, MapArt, FxParticleAlpha, FxParticleAdditive, FxParticleMultiply, FxRibbonAlpha, FxRibbonAdditive, FxRibbonMultiply, FxProjectorDecalAlpha, FxProjectorDecalAdditive, FxProjectorDecalMultiply, FxGlow, Shadow, AlphaShadow, OccluderDepth, SkinnedLit, SkinnedLocalLightAdd, SkinnedAlphaTestLit, SkinnedAlphaLit, SkinnedAddLit, SkinnedMultiplyLit, SkinnedUnlit, SkinnedWire, InstancedLit, InstancedLocalLightAdd, InstancedAlphaTestLit, InstancedAlphaLit, InstancedUnlit, InstancedWire, InstancedShadow, DynamicDetail, DynamicDetailShadow, TemporalAA, Fxaa, PostProcess, NameplateVisibility, ObjectVisibility;
    private readonly EffectMatrixVariable world, worldInvTranspose, viewProj, shadow0, shadow1, shadow2, shadow3, localProjectorInv, taaReprojection, skinPalette, fxProjectorFromWorld, fxProjectorInvViewProj;
    private readonly EffectVectorVariable camera, pointLight, frontLight, ambient, advanced, fog0, fog1, fogSky, fogParams, fogColorParams, shadowSplits, heightRange, worldSliceParams, overlay, materialFlatColor, materialBloomParams, opacityFadeParams, scrollOffset, scrollVisual, terrainChunkScale, materialUvScale, envBlend1, envBlend2, dydCameraRight, dydCreationDistances, dydParams, dydTextureInfo, mapArtTint, fxProjectorColor, fxProjectorUv, fxProjectorFalloff, fxProjectorHue0, fxProjectorHue1, fxProjectorHue2, fxProjectorHue3;
    ...
    public WorldEffect(Device device, string filename) : base(device, filename) {
      ...
      Height=FX.GetTechniqueByName("Height"); Wire=FX.GetTechniqueByName("Wire"); Water=FX.GetTechniqueByName("Water"); Overlay=FX.GetTechniqueByName("Overlay"); MapMarker=FX.GetTechniqueByName("MapMarker"); MapArt=FX.GetTechniqueByName("MapArt"); FxParticleAlpha=FX.GetTechniqueByName("FxParticleAlpha"); FxParticleAdditive=FX.GetTechniqueByName("FxParticleAdditive"); FxParticleMultiply=FX.GetTechniqueByName("FxParticleMultiply"); FxRibbonAlpha=FX.GetTechniqueByName("FxRibbonAlpha"); FxRibbonAdditive=FX.GetTechniqueByName("FxRibbonAdditive"); FxRibbonMultiply=FX.GetTechniqueByName("FxRibbonMultiply"); FxProjectorDecalAlpha=FX.GetTechniqueByName("FxProjectorDecalAlpha"); FxProjectorDecalAdditive=FX.GetTechniqueByName("FxProjectorDecalAdditive"); FxProjectorDecalMultiply=FX.GetTechniqueByName("FxProjectorDecalMultiply"); FxGlow=FX.GetTechniqueByName("FxGlow"); Shadow=FX.GetTechniqueByName("Shadow"); AlphaShadow=FX.GetTechniqueByName("AlphaShadow"); OccluderDepth=FX.GetTechniqueByName("OccluderDepth");
      ...
      localProjectorInv=FX.GetVariableByName("LocalLightProjectorInv").AsMatrix(); fxProjectorFromWorld=FX.GetVariableByName("FxProjectorFromWorld").AsMatrix(); fxProjectorInvViewProj=FX.GetVariableByName("FxProjectorInvViewProj").AsMatrix(); taaReprojection=FX.GetVariableByName("TaaReprojection").AsMatrix();
      ...
      heightRange=FX.GetVariableByName("HeightRange").AsVector(); worldSliceParams=FX.GetVariableByName("WorldSliceParams").AsVector(); overlay=FX.GetVariableByName("OverlayColor").AsVector(); materialFlatColor=FX.GetVariableByName("MaterialFlatColor").AsVector(); materialBloomParams=FX.GetVariableByName("MaterialBloomParams").AsVector(); opacityFadeParams=FX.GetVariableByName("OpacityFadeParams").AsVector(); scrollOffset=FX.GetVariableByName("ScrollingOffsetParams").AsVector(); scrollVisual=FX.GetVariableByName("ScrollingVisualParams").AsVector(); terrainChunkScale=FX.GetVariableByName("TerrainChunkScale").AsVector(); materialUvScale=FX.GetVariableByName("MaterialUvScale").AsVector(); envBlend1=FX.GetVariableByName("EnvBlendParams1").AsVector(); envBlend2=FX.GetVariableByName("EnvBlendParams2").AsVector();
      ...
      mapArtTint=FX.GetVariableByName("MapArtTint").AsVector(); fxProjectorColor=FX.GetVariableByName("FxProjectorColor").AsVector(); fxProjectorUv=FX.GetVariableByName("FxProjectorUv").AsVector(); fxProjectorFalloff=FX.GetVariableByName("FxProjectorFalloff").AsVector(); fxProjectorHue0=FX.GetVariableByName("FxProjectorHue0").AsVector(); fxProjectorHue1=FX.GetVariableByName("FxProjectorHue1").AsVector(); fxProjectorHue2=FX.GetVariableByName("FxProjectorHue2").AsVector(); fxProjectorHue3=FX.GetVariableByName("FxProjectorHue3").AsVector();
      ...
      enableLighting=FX.GetVariableByName("EnableLighting").AsScalar(); enableFog=FX.GetVariableByName("EnableFog").AsScalar(); enableShadows=FX.GetVariableByName("EnableShadows").AsScalar(); localCount=FX.GetVariableByName("LocalLightCount").AsScalar(); alphaMode=FX.GetVariableByName("AlphaMode").AsScalar(); alphaTest=FX.GetVariableByName("AlphaTestValue").AsScalar(); diffuseAlphaMax=FX.GetVariableByName("DiffuseTextureAlphaMax").AsScalar(); materialUsesEmissive=FX.GetVariableByName("MaterialUsesEmissive").AsScalar(); materialIsOpacityFade=FX.GetVariableByName("MaterialIsOpacityFade").AsScalar(); hookOpacity=FX.GetVariableByName("HookOpacity").AsScalar(); terrainUv=FX.GetVariableByName("TerrainUvScale").AsScalar(); nameplateDepthBias=FX.GetVariableByName("NameplateDepthBias").AsScalar(); viewerBlueGlow=FX.GetVariableByName("ViewerBlueGlow").AsScalar(); fxProjectorShape=FX.GetVariableByName("FxProjectorShape").AsScalar(); fxProjectorTwoSided=FX.GetVariableByName("FxProjectorTwoSided").AsScalar(); fxProjectorUseNormal=FX.GetVariableByName("FxProjectorUseNormal").AsScalar(); fxProjectorHueOn=FX.GetVariableByName("FxProjectorHueOn").AsScalar();
    ...
    public void SetWorldSlice(bool enabled,float height)=>worldSliceParams.Set(new Vector4(enabled?1f:0f,height,0f,0f));
    ...
    public void SetMapArt(ShaderResourceView texture,float opacity){SetMapArt(texture,opacity,new Vector4(1f,1f,1f,1f));}
    public void SetMapArt(ShaderResourceView texture,float opacity,Vector4 tint){mapArtTexture.SetResource(texture);mapArtOpacity.Set(Math.Max(0f,Math.Min(1f,opacity)));mapArtTint.Set(tint);}
    public void ClearMapArt(){mapArtTexture?.SetResource(null);mapArtOpacity?.Set(0f);mapArtTint?.Set(new Vector4(1f,1f,1f,1f));}

    // Screen-space FXSPEC projector receiver pass. Scene depth is sampled through the read-only DSV twin while the
    // decal is blended over the already-rendered world receiver. The projector transform maps world -> authored
    // unit box [-1,1]^3, matching Jedipedia/Projection_* semantics.
    public void SetFxProjectorDecal(ShaderResourceView depth, ShaderResourceView texture, Matrix inverseViewProj, Matrix projectorFromWorld,
        Vector4 color, Vector4 uvTileScroll, Vector4 falloff, Vector4 hue0, Vector4 hue1, Vector4 hue2, Vector4 hue3,
        int shape, bool twoSided, bool useNormal, bool hueOn) {
      sceneDepth.SetResource(depth); mapArtTexture.SetResource(texture); fxProjectorInvViewProj.SetMatrix(inverseViewProj); fxProjectorFromWorld.SetMatrix(projectorFromWorld);
      fxProjectorColor.Set(color); fxProjectorUv.Set(uvTileScroll); fxProjectorFalloff.Set(falloff);
      fxProjectorHue0.Set(hue0); fxProjectorHue1.Set(hue1); fxProjectorHue2.Set(hue2); fxProjectorHue3.Set(hue3);
      fxProjectorShape.Set(shape); fxProjectorTwoSided.Set(twoSided?1f:0f); fxProjectorUseNormal.Set(useNormal?1f:0f); fxProjectorHueOn.Set(hueOn?1f:0f);
    }
    public void ClearFxProjectorDecal(){sceneDepth?.SetResource(null);mapArtTexture?.SetResource(null);}
